...
class PGMQueue:
    ...
    def _initialize_sqlalchemy(
        ...
        if session_maker:
            ...
            self.session_maker = sessionmaker(bind=self.engine, class_=get_session_)
        else:
            self.engine = (
    ...
    @inject_session
    def _validate_queue_name_sync(
        self, queue_name: str, session: Optional[Session] = None
    ):
        """Validate the length of a queue name."""
        session.execute(*_statement.validate_queue_name(queue_name))

    @inject_async_session
    async def _validate_queue_name_async(
        self, queue_name: str, session: Optional[AsyncSession] = None
    ):
        """Validate the length of a queue name."""
    ...
    async def _drop_queue_async(
        ...
        """Drop a queue."""
        row = (
            await session.execute(*_statement.drop_queue(queue_name, partitioned))
        ).fetchone()
        if commit:
            await session.commit()
        return row[0]
    ...
    def list_queues(
        ...
        if self.is_async:
            return self.loop.run_until_complete(
                self._list_queues_async(session=session)
            )
    ...
    async def _send_async(
        ...
        row = (
            await session.execute(_statement.send(queue_name, message, delay))
        ).fetchone()
        if commit:
            await session.commit()
        return row[0]
    ...
    def _send_batch_sync(
        ...
        rows = (
            session.execute(_statement.send_batch(queue_name, messages, delay))
        ).fetchall()
        if commit:
            session.commit()
        return [row[0] for row in rows]
    ...
    async def _send_batch_async(
        ...
        rows = (
            await session.execute(_statement.send_batch(queue_name, messages, delay))
        ).fetchall()
        if commit:
            await session.commit()
        return [row[0] for row in rows]
    ...
    async def _read_batch_async(
        ...
        rows = (
            await session.execute(*_statement.read_batch(queue_name, vt, batch_size))
        ).fetchall()
        if commit:
            await session.commit()
        if not rows:
    ...
    async def _set_vt_async(
        ...
        """Set the visibility timeout for a message."""
        row = (
            await session.execute(*_statement.set_vt(queue_name, msg_id, vt_offset))
        ).fetchone()
        if commit:
            await session.commit()
        if row is None:
    ...
    @inject_session
    def _pop_sync(
        self, queue_name: str, session: Optional[Session] = None, commit: bool = True
    ) -> Optional[Message]:
    ...
    async def _archive_batch_async(
        ...
        rows = (
            await session.execute(_statement.archive_batch(queue_name, msg_ids))
        ).fetchall()
        if commit:
            await session.commit()
        return [row[0] for row in rows]
